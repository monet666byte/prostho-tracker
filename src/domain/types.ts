 ...
 * Removable/Fixed และ Design RPD) — ตัวนำเข้าชีตอ่านคอลัมน์เหล่านี้มาใส่ที่นี่
...
export interface StudentGates {
   ...
   * (OSCE กับสอบ RPD design ทำเป็นแค่ช่องติ๊กพอ) */
  osce?: boolean;
}
...
export interface Student {
   ...
   * (ปี 5/2569 → ปี 6/2570 พร้อมรับรุ่นใหม่เข้ามาปี 5 ในปีเดียวกัน)
...
export interface ClinicGroup {
  code: string; // TH-PT7
  /** กี่ท่านก็ได้ (เดิมเข้าใจว่า 2 ท่าน) */
  advisorIds: string[];
...
export interface AuditEntry {
  ...
  actorUid?: string;
  /**
   * ชนิดเหตุการณ์สำหรับแถวที่ต้องค้นเป็นชุด เช่น 'export' | 'retention'
   * ⚠️ ฝั่งแอปตั้งค่านี้เองไม่ได้ — trigger ใน migration 0016 ล้างทิ้งทุกครั้ง
   * มีแต่ฟังก์ชันบนเซิร์ฟเวอร์ (log_export / purge_expired_cohorts) ที่เขียนได้
   * ฟิลด์นี้จึงมีไว้ "อ่าน" อย่างเดียว — ค่าที่เห็นคือค่าที่เซิร์ฟเวอร์เขียนให้
   */
  kind?: string;
  /** รายละเอียดของเหตุการณ์ตาม kind (เช่น จำนวนแถวที่ export · ปิดบังชื่อไหม) */
...
export interface Requirement {
  ...
  /** Recall งานถอดได้ (CD/RPD) — ผู้ใช้ */
  recallRemovable: number;
  /** Recall งานติดแน่น (FDP) — ผู้ใช้ */
  recallFixed: number;
...
export interface Settings {
  ...
  /** ชั้นปีที่เปิดให้กรอกแบบประเมินตนเอง เช่น [5] เปิดเฉพาะปี 5 · [] คือปิดทั้งหมด
      (ภาคเปิดปีละครั้ง ตอนจบเทอม 1 — สองชั้นปีอาจไม่พร้อมพร้อมกัน จึงแยกเปิดได้) */
  saOpenYears: number[];
...
/* ── Section III · Knowledge & skill assessment ────────────────────────────
   ...
   ทำไมเก็บได้หลายแถวต่อใบ: ยังไม่ยืนยันกับภาคว่าประเมินซ้ำได้ไหม
