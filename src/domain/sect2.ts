 ...
 */
import type { Sect2Record } from './types';
 ...
 *
 * ภาคยืนยัน: **ประเมินก็ส่วนประเมิน · ที่ติ๊กผ่าน/ไม่ผ่านเองคือ
...
/**
 * ใบ Section II ประเมินเสร็จแล้วหรือยัง — ใบให้คะแนนดูที่ total · ใบ RPD design ดูที่ passed
 * แถวร่างกับแถวเสร็จหน้าตาเหมือนกันทุกอย่าง ต่างแค่ช่องนี้ · ทุกหน้า (นศ. · อจ. · ใบพิมพ์) ต้องใช้ตัวนี้
 */
export function isSect2Evaluated(r: Pick<Sect2Record, 'formKey' | 'passed' | 'total'> | undefined): boolean {
  if (!r) return false;
  return r.formKey === 'rpdDesign' ? r.passed !== undefined && r.passed !== null : r.total !== undefined && r.total !== null;
}
