...
export function importSheetCsv(csvText: string, studentId: string, entryYear?: number): ImportResult {
  ...
  dataRows.forEach((r, i) => {
    ...
    /* แถวที่ซ้ำกันทุกช่องในไฟล์เดียว (เกิดได้จริงเวลาคนก๊อปแถว) ต้องไม่ทับกันเอง —
       ...
       หน้านำเข้าประกาศกับอาจารย์ไว้เองว่า "แถวที่อ่านไม่ออกขึ้นรายงานให้คนตัดสิน ไม่เดาเอง" */
    ...
    if (nth > 0) {
      issues.push({
        row: rowNo,
        column: 'Prosthodontic work',
        value: workLabel,
        problem: `ซ้ำกับแถวก่อนหน้าทุกช่อง (ผู้ป่วย · ชิ้นงาน · วันรับเคส) — นำเข้าเป็น ${nth + 1} เคสแยกกัน ถ้าก๊อปแถวพลาดจะนับเข้าเกณฑ์เกินจริง`,
      });
    }
    workpieces.push({
...
export function importGroupCsv(
  ...
  /* ══ ไฟล์นี้ export มาถูกทางหรือเปล่า (เพิ่ม 10 ก.ย. 69) ══════════════════════
     ...
     วัดกับชีตจริงรุ่น 55 ทั้ง 12 แท็บ:
       ...
       ทาง                         ชิ้นงาน   ไม่มีวันรับเคส   อ่านประเภทไม่ออก
       Google Sheets → CSV           431       203 (47%)          0
       แปลงผ่าน .xlsx ก่อน           396       395 (99.7%)        5

     ทางที่แปลงผ่าน .xlsx **ทำคอลัมน์วันรับเคสหายทั้งคอลัมน์** และหายไป 35 ชิ้นงาน
     แต่ตัวนำเข้ายังทำงานต่อได้ (มันสนิฟช่องติ๊กจากข้อมูลจริงเป็นแผนสำรอง)
     อาจารย์จึงเห็นแค่รายงานว่า "ไม่ได้กรอกวันรับเคส 395 แถว" ซึ่งอ่านแล้วเข้าใจว่า
     "ชีตกรอกไม่ครบ" ไม่ใช่ "คุณ export มาผิดทาง" — คนละเรื่องกันคนละการแก้

     ตัวชี้ที่ชัดและไม่กำกวม: ทาง .xlsx เขียนหัวช่องติ๊กเป็น 0.0 … 10.0 (ทศนิยม)
     ส่วน Google เขียน 0 … 10 ตรง ๆ */
  const tickHeads = header.map((c) => (c ?? '').trim()).filter((c) => /^\d+\.0$/.test(c));
  if (tickHeads.length >= 5) {
    fileIssues.push({
      row: 0,
      column: 'ทั้งไฟล์',
      value: tickHeads.slice(0, 3).join(' / '),
      problem: 'ไฟล์นี้ดูเหมือนถูกแปลงผ่าน .xlsx ก่อน (หัวช่องติ๊กเป็น 0.0–10.0) — วิธีนี้ทำให้คอลัมน์วันรับเคสหายทั้งคอลัมน์และชิ้นงานหายบางส่วน โปรด export จาก Google Sheets โดยตรง (ไฟล์ → ดาวน์โหลด → CSV ทีละแท็บ) แล้วนำเข้าใหม่',
    });
  }
