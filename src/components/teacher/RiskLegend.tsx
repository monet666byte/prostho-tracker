      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: 'grid', gap: 7 }}>
            <Row level="ok" title={t('เขียว')} body={t('งานที่เหลือใช้ไม่ถึง 70% ของคาบที่เหลือ — มีที่หายใจ')} />
            <Row level="medium" title={t('ส้ม')} body={t('ใช้ 70–100% ของคาบที่เหลือ — ทันแต่ห้ามหยุด')} />
            <Row
              level="high"
              title={t('แดง')}
              body={t('คาบที่เหลือไม่พอตามจังหวะเฉลี่ย · หรือยังไม่มีเคสในมือเลย · หรือมาคลินิกแล้ว step ไม่ขยับ 3 คาบติด')}
            />
          </div>
          ...
          <div style={{ marginTop: 11, paddingTop: 10, borderTop: '1px solid var(--divider)' }}>
            ...
            <div className="pretty" style={{ font: '400 11px/1.7 var(--font-body)', color: 'var(--text-muted)' }}>
              ...
              {t('③ เทียบกับคาบที่เหลือถึงปลายปีการศึกษา — ตอนนี้ ~{n} คาบ (เส้นส้มที่ ~{a} คาบ)', { n: left, a: amber })}
            </div>
