      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: 'grid', gap: 7 }}>
            {/* สองสี (ผู้ใช้เลือก 16 ก.ย. 69) — ส้มเดิมรวมเข้าแดง "ต้องตาม" · กติกาคำนวณ 3 ระดับเหมือนเดิม */}
            <Row level="ok" title={t('เขียว')} body={t('ยังเหลือคาบเผื่อสบายๆ (เกิน 30% ของที่มี)')} />
            <Row
              level="high"
              title={t('แดง · ต้องตาม')}
              body={t('เหลือเผื่อไม่ถึง 30% · หรืองานกินคาบเกินที่มี · หรือยังไม่มีเคสในมือ · หรือมาคลินิกแล้ว step ไม่ขยับ 3 คาบติด')}
            />
          </div>
          ...
          <div style={{ marginTop: 11, paddingTop: 10, borderTop: '1px solid var(--divider)' }}>
            ...
            <div className="pretty" style={{ font: '400 11px/1.7 var(--font-body)', color: 'var(--text-muted)' }}>
              ...
              {t('③ เทียบกับคาบที่เหลือถึงปลายปีการศึกษา — ตอนนี้ ~{n} คาบ (เกิน ~{a} คาบเมื่อไหร่ = แดง)', { n: left, a: amber })}
            </div>
            ...
            <div style={{ marginTop: 8, font: '400 10.5px/1.6 var(--font-body)', color: 'var(--text-faint)' }}>
              {t('⚠️ เลขคาบเป็นค่าประมาณจากสมมติฐาน (คาบ/สัปดาห์ ปรับได้ที่ “ตั้งค่า”) — ใช้จัดลำดับว่าควรดูใครก่อน ไม่ใช่คำตัดสิน')}
            </div>
          </div>
        </div>
      )}
