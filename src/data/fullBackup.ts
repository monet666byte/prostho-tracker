 ...
 *
 * ทำไมต้องมี: ตัวสำรองข้อมูลที่มีอยู่คือ `npm run backup` ซึ่งต้องมี
 * terminal + `.env.local` + บัญชีหัวหน้าภาค — ไม่มีใครในภาครันได้ · สำเนาชุดล่าสุด
 * ทำเป็นครั้งคราวด้วยมือ ไม่มีตั้งเวลา · และ Supabase แผนฟรีไม่มี backup อัตโนมัติ
 ...
 *
 * เนื้อในคือ "ทุกอย่างที่เครื่องนี้มี" — ซึ่งสำหรับอาจารย์คือข้อมูลนักศึกษาทั้งชั้นปี
 * (RLS ให้อาจารย์เห็นผู้ป่วย/ชิ้นงานทุกแถว · audit เห็นตามขอบเขตของ 0021/0024) และ local-first
