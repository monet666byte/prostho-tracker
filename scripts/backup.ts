...
/*
 ...
 ⚠️ โฟลเดอร์สำเนาคือข้อมูลผู้ป่วย + รูปในปากคนไข้ แบบไม่เข้ารหัส — เก็บในดิสก์ที่เข้ารหัส
    (FileVault) หรือสื่อออฟไลน์ที่ล็อกได้ ห้ามซิงก์ขึ้นคลาวด์ส่วนตัว และห้ามอยู่ในโฟลเดอร์ที่ git เห็น

 เปิด Terminal แล้วพิมพ์:   crontab -e
 เพิ่มบรรทัดนี้ (สำรองทุกวัน 21:00 น. — เครื่องต้องเปิดอยู่)
 แทน <โฟลเดอร์ repo> ด้วยที่อยู่จริงของโฟลเดอร์โปรเจกต์ · cron ไม่รู้จัก PATH ของ shell จึงต้องใส่ npx แบบเต็ม:

   0 21 * * * cd "<โฟลเดอร์ repo>" && "$(which npx)" tsx scripts/backup.ts >> backups/cron.log 2>&1

 (พิมพ์ `which npx` ใน Terminal ก่อน แล้วเอาที่อยู่ที่ได้ไปวางแทน "$(which npx)" ถ้า cron ของเครื่องนั้นไม่ขยายให้)
