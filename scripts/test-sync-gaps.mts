/**
 * เทสต์ชุดที่ 23 — "รูในตาข่าย" ที่ `npm run mutate:sync` หาเจอ · รันด้วย `npm run test:sync-gaps`
 *
 * ทำไมต้องมี (13 ก.ย. 69): ใส่บั๊กที่เคยเกิดจริง/เกือบเกิด 41 จุดลง cloudSync.ts ทีละจุด
 * เทสต์เดิม 1,041 ข้อจับได้ 24 จุด · อีก 11 จุดเป็นบั๊กที่ทำข้อมูลหายได้จริงแต่ไม่มีเทสต์ไหนร้อง
