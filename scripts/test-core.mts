...
/* caseCode() ต้องอ่านแค่ id เท่านั้น ไม่แอบอ่านชื่อหรือ HN
   (ส่วนที่ว่า id ของผู้ป่วยนำเข้า *ถูกสร้าง* จาก HN เป็นคนละเรื่อง — ดูบล็อกข้างล่าง) */
{
  const a = patientLabel({ id: 'pt-a', name: 'สมชาย ใจดี', hn: 'HN-1' }, 'code').name;
  const b = patientLabel({ id: 'pt-a', name: 'สมหญิง ใจงาม', hn: 'HN-9999' }, 'code').name;
  ok('id เดียวกัน แต่คนละชื่อคนละ HN → ต้องได้รหัสเดียวกัน (รหัสไม่ได้อ่านสองช่องนั้น)', a === b, `${a} vs ${b}`);
}
...
/* ── ข้อจำกัดที่รู้ตัว: ผู้ป่วยที่นำเข้าจากชีต รหัสเคสคำนวณย้อนได้ ─────────────
   ...
   (README หัวข้อ PDPA ข้อ 10) เทสต์นี้จะพังทันที ให้กลับมาแก้ทั้งเทสต์และหัวไฟล์ privacy.ts
   ⚠️ ห้าม "แก้เทสต์ให้ผ่าน" เฉย ๆ — มันคือสัญญาณว่าคำอธิบายเรื่องความเป็นส่วนตัวเปลี่ยนไปแล้ว */
{
  ...
  ok('HN ต่างกันหนึ่งหลัก ได้รหัสคนละตัว (ยืนยันว่าตารางเทียบใช้ได้จริง)',
    caseCode(importedId('st-6504001', '66-01234')) !== caseCode(importedId('st-6504001', '66-01235')));
}
