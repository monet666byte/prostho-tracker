/**
 * "ทำงานตอนไม่มีเน็ต แล้วปิดแท็บ" · รันด้วย `npm run test:offline`
 *
...
console.log('\nสำเนาคิวรูปเก่าบนดิสก์ ต้องยังอ่านออก');
{
  ...
  /* ⚠️ ต้องใส่ `cloudBoundUid` ด้วย ไม่ใช่แค่คิว — ไม่งั้น `bindToUser` อ่านว่า
     ...
     ค่าคือ `${uid}@${POLICY_VERSION}` — auth ปลอมคืน uid 'u1'
     ⭐ ตั้งใจใช้เลขรุ่นเก่า ('v2') — เครื่องที่อัปเดตแอปกลางคาบคือเครื่องที่มีทั้งคิวรูปเก่า *และ* เลขรุ่นสิทธิ์เก่า
     bindToUser ต้องส่งงานค้างของคนเดิมขึ้นก่อนค่อยล้างลิ้นชัก · ถ้าเทสต์นี้ตกหลังขยับ POLICY_VERSION
     = งานค้างส่งของผู้ใช้หายตอนอัปเดต ห้ามแก้ที่ fixture */
