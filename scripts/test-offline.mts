...
console.log('\nสำเนาคิวรูปเก่าบนดิสก์ ต้องยังอ่านออก');
{
  ...
  /* ⚠️ ต้องใส่ `cloudBoundUid` ด้วย ไม่ใช่แค่คิว — ไม่งั้น `bindToUser` อ่านว่า
     ...
     ค่าคือ `${uid}@${POLICY_VERSION}` — auth ปลอมคืน uid 'u1' · POLICY_VERSION ปัจจุบันคือ 'v3' (cloudSync.ts)
     ขยับเลขรุ่นใน cloudSync เมื่อไหร่ต้องมาแก้ตรงนี้ด้วย ไม่งั้นเทสต์ตกเพราะ fixture */
