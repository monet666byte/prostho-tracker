...
2. **นำร่องเซิร์ฟเวอร์กลางกับคนจริง** — โครงสร้างทำครบแล้ว: Supabase + local-first sync (`data/cloudSync.ts`),
   RLS ต่อแถว, audit log, migration `0001`–`0014` รันครบแล้ว (8 ก.ย. 69)
   **แต่ยังไม่เคยให้ผู้ใช้จริงลองพร้อมกันหลายเครื่อง** — ข้อ 3 กับ 4 ข้างล่างคือสิ่งที่ต้องเคลียร์ก่อนถึงตรงนั้น
...
- เทสต์ยังไม่ครอบทั้งระบบ — มี 3 ชุด (`npm test`): `domain/rules.ts` (เกณฑ์/เคสค้าง/% completed),
  ค่าตั้งของภาค และการอ่านชั้นปีจากรหัสกลุ่ม · ส่วน UI ยังไม่มีเทสต์ ต้องลองใช้จริงเอา
